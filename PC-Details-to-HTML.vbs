...
'Updated 12/04/2012 to get the OS version.
'Updated 25/06/2012 to correctly ID Dell Service Tag as such, not Serial Number. Minor bug fix.
...
For each objBIOS in colBIOS
      BInfoMan = objBIOS.Manufacturer
    BInfo = "<strong>" & "Manufacturer: " & "</strong>" & BInfoMan & "</br>" & vbCrLf
      Select Case BInfoMan
      Case "Dell Inc."
            BInfo = BInfo & "<strong>" & "Dell Service Tag: " & "</strong>" & objBIOS.SerialNumber & "</br>"
      Case "Hewlett-Packard"
            BInfo = BInfo & "<strong>" & "Serial number: " & "</strong>" & objBIOS.SerialNumber & "</br>"
      Case Else
            BInfo = BInfo & "<strong>" & "BIOS Serial Number field: " & "</strong>" & objBIOS.SerialNumber & "</br>"
      End Select
Next
BiosInfo = BInfo
End Function
...
Function GetOSVersion(OSorALL)
'Function to Get the OS Version, Service Pack, etc.
dim OSVers
OSVers = ""
Set objWMI = GetObject("winmgmts:\\.\root\CIMV2")
Set colItems = objWMI.ExecQuery("SELECT * FROM Win32_OperatingSystem",,48)
For Each objItem in colItems
    OSCap = objItem.Caption
    OSVers = OSCap & "</br>" & "Version: " & objItem.Version & "</br>"
    OSVers = OSVers & "ServicePack: " & objItem.ServicePackMajorVersion & "</br>"
Next
'MsgBox(OSVers)
'GetOSVersion = OSVers
Select case OSorALL
      ...
      Case "OSCap"
            GetOSVersion = OSCap
End Select
'Call like "OSVersion = GetOSVersion("All")
End Function
...
dim Report, WordArray, NameOfUser, MachineModel, FName, Title, Notes, OSCaptionCheck
...
Report = Report & "<strong>" & "Misc Notes: " & "</strong>" & Notes & "</br> </br>" & VbCrLf
End if      'End of "Notes" if.
'OS Version
Report = Report & "<strong> OS: " & GetOSVersion("All") & "</strong>" & "</br>" & VbCrLf
...
OSCaptionCheck = GetOSVersion("OSCap")
If (Left(OSCaptionCheck, 19)) = "Microsoft Windows 7" then
      FName = "C:\Users\" & UserHostName("Username") & "\" & Title & ".html"
Else
      FName = "C:\" & Title & ".html"
End If
